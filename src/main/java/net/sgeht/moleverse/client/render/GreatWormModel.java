...
/**
 * Great worm model: sixteen segments and a crawl that is a formula, not keyframes.
 *
...
public class GreatWormModel extends EntityModel<LivingEntityRenderState> {
     ...
     *
     * <p>Sixteen segments, doubled when the worm sheet's variant C won with
     * the note that the animal was far too short. The chain only improves
     * with the count: the peristaltic pulse is per-segment and compounds
     * nothing, and the lateral S keeps one period over the whole body
     * ({@link #LATERAL_PHASE} divides by this array's length), so the yaw
     * each joint takes actually <em>fell</em> with the doubling - a longer
     * chain is a smoother curve, never a spring.</p>
     */
     ...
     * turn puts a full contraction wave across four segments - twenty-eight
     * units of body - so about four of them ride the doubled body at once,
     * each looking exactly as the old worm's two did. Larger and the whole
     * animal pulses as one; smaller and the segments alternate and it reads
     * as a caterpillar.
     */
     ...
     *
     * <p>Raised from 2.2 as the body doubled: 127 units of worm can wind
     * three off its line and still read as one animal in a corridor. The yaw
     * per joint still fell with the change - {@link #segmentYaw} divides the
     * phase step by the segment count, 0.25 radians before, 0.17 now - which
     * is the checked property that keeps the longer chain smooth.</p>
     */
    private static final float SWAY = 3.0F;
    ...
    // --- generated by great_worm_shape.py --java, do not edit --------------
    ...
    // Variant C's features ride their bones as extra boxes: a three-step
    // prostomium and the mouth under its overhang on the head, the clitellum
    // saddle on body5, a raised annulus on every even body segment sitting on
    // the texture's own three unit ring grid, and the flat tail paddle on the
    // tail. The Blockbench export in art/ is the cross-check for this body,
    // not its source - see the emitter's note.
    public static LayerDefinition createBodyLayer() {
        ...
        root.addOrReplaceChild("head", CubeListBuilder.create()
                .texOffs(44, 133).addBox(-6.0F, -9.0F, -4.5F, 12.0F, 9.0F, 9.0F, new CubeDeformation(0.0F))
                .texOffs(108, 0).addBox(-3.0F, -5.0F, -7.5F, 6.0F, 4.0F, 4.0F, new CubeDeformation(0.0F))
                .texOffs(108, 24).addBox(-2.0F, -5.0F, -10.5F, 4.0F, 3.0F, 3.0F, new CubeDeformation(0.0F))
                .texOffs(118, 151).addBox(-1.0F, -5.0F, -12.5F, 2.0F, 2.0F, 2.0F, new CubeDeformation(0.0F))
                .texOffs(108, 48).addBox(-4.0F, -3.0F, -6.0F, 8.0F, 3.0F, 2.0F, new CubeDeformation(0.0F)),
                PartPose.offset(0.0F, 0.0F, -52.5F));

        root.addOrReplaceChild("body1", CubeListBuilder.create()
                .texOffs(0, 114).addBox(-6.0F, -10.0F, -4.5F, 12.0F, 10.0F, 9.0F, new CubeDeformation(0.0F)),
                PartPose.offset(0.0F, 0.0F, -45.5F));

        root.addOrReplaceChild("body2", CubeListBuilder.create()
                .texOffs(0, 93).addBox(-7.0F, -12.0F, -4.5F, 14.0F, 12.0F, 9.0F, new CubeDeformation(0.0F))
                .texOffs(82, 151).addBox(-8.0F, -13.0F, -1.5F, 16.0F, 13.0F, 2.0F, new CubeDeformation(0.0F)),
                PartPose.offset(0.0F, 0.0F, -38.5F));

        root.addOrReplaceChild("body3", CubeListBuilder.create()
                .texOffs(0, 48).addBox(-9.0F, -14.0F, -4.5F, 18.0F, 14.0F, 9.0F, new CubeDeformation(0.0F)),
                PartPose.offset(0.0F, 0.0F, -31.5F));

        root.addOrReplaceChild("body4", CubeListBuilder.create()
                .texOffs(0, 0).addBox(-9.0F, -15.0F, -4.5F, 18.0F, 15.0F, 9.0F, new CubeDeformation(0.0F))
                .texOffs(84, 114).addBox(-10.0F, -16.0F, -0.5F, 20.0F, 16.0F, 2.0F, new CubeDeformation(0.0F)),
                PartPose.offset(0.0F, 0.0F, -24.5F));

        root.addOrReplaceChild("body5", CubeListBuilder.create()
                .texOffs(54, 0).addBox(-9.0F, -15.0F, -4.5F, 18.0F, 15.0F, 9.0F, new CubeDeformation(0.0F))
                .texOffs(0, 71).addBox(-10.0F, -16.0F, -3.0F, 20.0F, 16.0F, 6.0F, new CubeDeformation(0.0F)),
                PartPose.offset(0.0F, 0.0F, -17.5F));

        root.addOrReplaceChild("body6", CubeListBuilder.create()
                .texOffs(0, 24).addBox(-9.0F, -15.0F, -4.5F, 18.0F, 15.0F, 9.0F, new CubeDeformation(0.0F))
                .texOffs(0, 133).addBox(-10.0F, -16.0F, 0.5F, 20.0F, 16.0F, 2.0F, new CubeDeformation(0.0F)),
                PartPose.offset(0.0F, 0.0F, -10.5F));

        root.addOrReplaceChild("body7", CubeListBuilder.create()
                .texOffs(54, 24).addBox(-9.0F, -15.0F, -4.5F, 18.0F, 15.0F, 9.0F, new CubeDeformation(0.0F)),
                PartPose.offset(0.0F, 0.0F, -3.5F));

        root.addOrReplaceChild("body8", CubeListBuilder.create()
                .texOffs(54, 48).addBox(-9.0F, -14.0F, -4.5F, 18.0F, 14.0F, 9.0F, new CubeDeformation(0.0F))
                .texOffs(0, 151).addBox(-10.0F, -15.0F, -1.5F, 20.0F, 15.0F, 2.0F, new CubeDeformation(0.0F)),
                PartPose.offset(0.0F, 0.0F, 3.5F));

        root.addOrReplaceChild("body9", CubeListBuilder.create()
                .texOffs(52, 71).addBox(-8.0F, -13.0F, -4.5F, 16.0F, 13.0F, 9.0F, new CubeDeformation(0.0F)),
                PartPose.offset(0.0F, 0.0F, 10.5F));

        root.addOrReplaceChild("body10", CubeListBuilder.create()
                .texOffs(46, 93).addBox(-7.0F, -12.0F, -4.5F, 14.0F, 12.0F, 9.0F, new CubeDeformation(0.0F))
                .texOffs(0, 168).addBox(-8.0F, -13.0F, -0.5F, 16.0F, 13.0F, 2.0F, new CubeDeformation(0.0F)),
                PartPose.offset(0.0F, 0.0F, 17.5F));

        root.addOrReplaceChild("body11", CubeListBuilder.create()
                .texOffs(42, 114).addBox(-6.0F, -10.0F, -4.5F, 12.0F, 10.0F, 9.0F, new CubeDeformation(0.0F)),
                PartPose.offset(0.0F, 0.0F, 24.5F));

        root.addOrReplaceChild("body12", CubeListBuilder.create()
                .texOffs(86, 133).addBox(-5.0F, -9.0F, -4.5F, 10.0F, 9.0F, 9.0F, new CubeDeformation(0.0F))
                .texOffs(70, 168).addBox(-6.0F, -10.0F, 0.5F, 12.0F, 10.0F, 2.0F, new CubeDeformation(0.0F)),
                PartPose.offset(0.0F, 0.0F, 31.5F));

        root.addOrReplaceChild("body13", CubeListBuilder.create()
                .texOffs(44, 151).addBox(-5.0F, -8.0F, -4.5F, 10.0F, 8.0F, 9.0F, new CubeDeformation(0.0F)),
                PartPose.offset(0.0F, 0.0F, 38.5F));

        root.addOrReplaceChild("body14", CubeListBuilder.create()
                .texOffs(92, 93).addBox(-4.0F, -7.0F, -4.5F, 8.0F, 7.0F, 9.0F, new CubeDeformation(0.0F))
                .texOffs(102, 71).addBox(-5.0F, -8.0F, -1.5F, 10.0F, 8.0F, 2.0F, new CubeDeformation(0.0F)),
                PartPose.offset(0.0F, 0.0F, 45.5F));

        root.addOrReplaceChild("tail", CubeListBuilder.create()
                .texOffs(36, 168).addBox(-4.0F, -6.0F, -4.5F, 8.0F, 6.0F, 9.0F, new CubeDeformation(0.0F))
                .texOffs(0, 183).addBox(-5.0F, -3.0F, 3.5F, 10.0F, 3.0F, 6.0F, new CubeDeformation(0.0F)),
                PartPose.offset(0.0F, 0.0F, 52.5F));
