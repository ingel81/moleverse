...
public class MoleModel extends EntityModel<MoleRenderState> {
    ...
    // --- generated by Blockbench, hero-pass additions marked ----------------
    ...
    //
    // The hero pass is STRICTLY ADDITIVE: every original bone keeps its name,
    // pivot and cubes byte-identical, because the tuned animations reference
    // them, and every new cube rides an existing bone (or, for the ears, a new
    // child bone the animations never need to know about). New strips live in
    // the atlas's new lower half; their offsets are copied from
    // art/generators/mole_refine.py, which owns them. The silhouette rule:
    // nothing new stands more than one texel outside the tuned outline.
    public static LayerDefinition createBodyLayer() {
        ...
        PartDefinition body = root.addOrReplaceChild("body", CubeListBuilder.create()
                ...
                // Fur tufts: thin boxes breaking the back's ruler edge, half a
                // texel proud, one on the chest and two along the rump.
                .texOffs(0, 38).addBox(-1.5F, -3.0F, -3.5F, 2.0F, 1.0F, 1.0F, new CubeDeformation(0.0F))
                .texOffs(8, 38).addBox(-2.5F, -3.5F, 0.0F, 2.0F, 1.0F, 1.0F, new CubeDeformation(0.0F))
                .texOffs(16, 38).addBox(0.5F, -3.5F, 2.5F, 2.0F, 1.0F, 1.0F, new CubeDeformation(0.0F)),
        ...
        body.addOrReplaceChild("tail", CubeListBuilder.create()
                ...
                // A tuft on the stub's end, so the tail stops in fur, not in a
                // sawn-off face.
                .texOffs(44, 36).addBox(-0.5F, -1.5F, 3.5F, 1.0F, 1.0F, 1.0F, new CubeDeformation(0.0F)),
        ...
        // Ear nubs as child bones of the head: they inherit every head channel
        // for free, and being bones of their own leaves the door open for an
        // ear flick later without touching any animation that exists today.
        head.addOrReplaceChild("ear_left", CubeListBuilder.create()
                .texOffs(32, 36).addBox(-0.5F, -0.5F, -0.5F, 1.0F, 1.0F, 1.0F, new CubeDeformation(0.0F)),
                PartPose.offset(1.8F, -1.2F, -0.8F));

        head.addOrReplaceChild("ear_right", CubeListBuilder.create()
                .texOffs(38, 36).addBox(-0.5F, -0.5F, -0.5F, 1.0F, 1.0F, 1.0F, new CubeDeformation(0.0F)),
                PartPose.offset(-1.8F, -1.2F, -0.8F));
        ...
        head.addOrReplaceChild("snout", CubeListBuilder.create()
                ...
                // Whisker studs, two a side, half a texel proud of the muzzle:
                // the vibrissae a digging animal actually steers by.
                .texOffs(32, 32).addBox(1.0F, 0.5F, -3.0F, 1.0F, 1.0F, 1.0F, new CubeDeformation(0.0F))
                .texOffs(38, 32).addBox(1.0F, 1.0F, -2.0F, 1.0F, 1.0F, 1.0F, new CubeDeformation(0.0F))
                .texOffs(44, 32).addBox(-2.0F, 0.5F, -3.0F, 1.0F, 1.0F, 1.0F, new CubeDeformation(0.0F))
                .texOffs(50, 32).addBox(-2.0F, 1.0F, -2.0F, 1.0F, 1.0F, 1.0F, new CubeDeformation(0.0F)),
        ...
        frontLegRight.addOrReplaceChild("front_paw_right", CubeListBuilder.create()
                ...
                // Two staggered ridges between the original three: five claws
                // to a shovel, the defining feature, set half a texel back and
                // down so the comb reads as a comb.
                .texOffs(0, 32).addBox(-2.0F, 0.75F, -4.5F, 1.0F, 1.0F, 2.0F, new CubeDeformation(0.0F))
                .texOffs(8, 32).addBox(-0.5F, 0.75F, -4.5F, 1.0F, 1.0F, 2.0F, new CubeDeformation(0.0F)),
        ...
        frontLegLeft.addOrReplaceChild("front_paw_left", CubeListBuilder.create()
                ...
                .texOffs(16, 32).addBox(-0.5F, 0.75F, -4.5F, 1.0F, 1.0F, 2.0F, new CubeDeformation(0.0F))
                .texOffs(24, 32).addBox(1.0F, 0.75F, -4.5F, 1.0F, 1.0F, 2.0F, new CubeDeformation(0.0F)),
                PartPose.offsetAndRotation(0.0F, 3.0F, 0.0F, 0.0F, -0.3142F, 0.2094F));
